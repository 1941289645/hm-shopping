<template>
  <div class="login">
    ...
    <div class="container">
      ...
      <div class="form">
        <div class="form-item">
          <input v-model="mobile" type="text" class="inp" placeholder="请输入手机号码" maxlength="11">
        </div>
        <div class="form-item">
        ...
        <div class="form-item">
          ...
          <button @click="getCode">
            {{ second===totalSencond ? '获取验证码' : second+'秒后重新发送'}}
          </button>
        </div>
      </div>
...
export default {
  ...
  data () {
    return {
      picKey: '', // 图形验证码的key
      picUrl: '', // 图形验证码的地址
      totalSencond: 60, // 倒计时总时间
      second: 60, // 当前秒数，开定时器对其进行减1操作
      timer: null, // 定时器id
      mobile: '', // 手机号
      picCode: '', // 用户输入的图形验证码
      smsCode: '' // 用户输入的短信验证码
